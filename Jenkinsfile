...
pipeline {
 ...
 stages {
        ...
        stage('Checkout-git kubespray'){
               steps{
		sh '''
                cd kubespray && rm -rf * & git clone git@github.com:danibasconCHAKRAY/kubespray.git
               '''
		}
        }
        ...
        stage('RunApp') {
            steps {
            	sh '''
            		bash -c "source entorno_virtual/bin/activate && ${WORKSPACE}/entorno_virtual/bin/python  ${WORKSPACE}/../k8s/vagrant2ansible.py > host"
                '''
            }
        } 
  }
